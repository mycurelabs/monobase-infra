...
resource "aws_iam_role" "external_secrets" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${replace(aws_eks_cluster.main.identity[0].oidc[0].issuer, "https://", "")}:sub" = "system:serviceaccount:external-secrets-system:external-secrets"
...
# IRSA Role: cert-manager (for Route53 DNS-01 challenge)
