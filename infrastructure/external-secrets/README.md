...
File: `gcp-secretstore.yaml`

Generated by `scripts/secrets-gcp.sh` with:
...
SOPS support has been removed from this repository. If you have legacy SOPS secrets:

1. Decrypt: `sops -d secrets.enc.yaml > secrets.yaml`
2. Import to cloud KMS: Use `scripts/secrets-gcp.sh` import feature
...
- `gcp-secretstore.yaml` - GCP Secret Manager ClusterSecretStore (generated by script)
- `aws-secretstore.yaml` - AWS Secrets Manager ClusterSecretStore (not yet implemented)
- `azure-secretstore.yaml` - Azure Key Vault ClusterSecretStore (not yet implemented)
