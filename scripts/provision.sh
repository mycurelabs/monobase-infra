...
if [[ ! -d "$CLUSTER_DIR" ]]; then
    ...
    echo ""
    print_info "Create cluster configuration first by copying an example:"
    echo "  # For AWS EKS:"
    echo "  cp -r clusters/example-aws-eks clusters/$CLUSTER_NAME"
    echo ""
    echo "  # For DigitalOcean DOKS:"
    echo "  cp -r clusters/example-do-doks clusters/$CLUSTER_NAME"
    echo ""
    echo "  # For local k3d:"
    echo "  cp -r clusters/example-k3d clusters/$CLUSTER_NAME"
    echo ""
    echo "  # Then customize:"
...
if [[ "$MERGE_KUBECONFIG" == "true" ]] && [[ -n "$KUBECONFIG_PATH" ]]; then
    ...
    echo "   mkdir deployments/myclient-prod"
    echo "   cp deployments/templates/production-base.yaml deployments/myclient-prod/values-production.yaml"
    echo "   vim deployments/myclient-prod/values-production.yaml  # Edit domain, namespace, etc."
    echo ""
    echo "3. Bootstrap GitOps auto-discovery (ONE-TIME):"
    echo "   ./scripts/bootstrap.sh"
    echo ""
    echo "4. Commit and push to deploy:"
    echo "   git add deployments/myclient-prod/"
