...
Set the reserved IP by name on the Gateway's provisioned Service via `infrastructure.annotations` in `values/infrastructure/main.yaml` (see [static-ip-prerequisites.md](./static-ip-prerequisites.md)):
...
- Ensure the annotation references the correct IP name
