...
helm template pg-logical-backup charts/pg-logical-backup \
  -f values/deployments/mycure-preprod.yaml \
  --set pgLogicalBackup.enabled=true \
  --set pgLogicalBackup.restoreVerify.enabled=true \
...
- **Least privilege.** The dump runs as a dedicated read-only `dumper` role
  (`pg_read_all_data`), provisioned by a PreSync hook on the primary. No password
  ever appears on a command line: the superuser password is passed via
  `PGPASSWORD`, the dumper password via `psql \getenv`.
